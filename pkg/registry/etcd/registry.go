...
var (
	_ internal.Registrar = (*Registry)(nil)
	_ internal.Discovery = (*Registry)(nil)
)
...
func (r *Registry) Register(ctx context.Context, service *internal.ServiceInstance) error {
...
func (r *Registry) Deregister(ctx context.Context, service *internal.ServiceInstance) error {
	defer func() {
		if r.lease != nil {
			r.lease.Close()
		}
...
func (r *Registry) GetService(ctx context.Context, name string) ([]*internal.ServiceInstance, error) {
	key := fmt.Sprintf("%s/%s", r.opts.namespace, name)
	resp, err := r.kv.Get(ctx, key, clientv3.WithPrefix())
	if err != nil {
		return nil, err
	}
	items := make([]*internal.ServiceInstance, 0, len(resp.Kvs))
...
func (r *Registry) Watch(ctx context.Context, name string) (internal.ServiceWatcher, error) {
	key := fmt.Sprintf("%s/%s", r.opts.namespace, name)
